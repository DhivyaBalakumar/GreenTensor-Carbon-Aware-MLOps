"""
GreenTensor Report Generator
Generates formatted terminal reports for ML training runs.
...
def generate_report(
    ...
    lines = [
        "",
        "  +======================================+",
        "  |        GreenTensor Report  v0.7.0    |",
        "  +======================================+",
        "  Runtime          : {:.2f} s".format(duration),
        "  Energy           : {:.6f} kWh".format(energy_kwh),
    ...
    if baseline is not None:
        ...
        reduction_pct = (energy_saved / baseline.energy_kwh * 100) if baseline.energy_kwh > 0 else 0.0
    ...
    if not footprint or footprint.is_clean:
        ...
        if footprint.network_connections:
            lines.append("")
            lines.append("  Network connections observed: {}".format(
                ", ".join(footprint.network_connections[:5])))
        if footprint.child_processes:
            lines.append("  Child processes observed: {}".format(
